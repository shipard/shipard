 ...
 * @package e10pro\zus
 */
class PlanDaily extends Utility
{
	var $teacherNdx;
	var $tableHodiny;
	var $znamkyHodnoceni;
	...
	public function loadTimetable ()
	{
		...
		array_push ($q, ' ORDER BY pobocky.id, rozvrh.ucebna, rozvrh.zacatek, rozvrh.ndx');
...
divs.on( 'scroll', sync);
	...
	public function createToPlan()
	{
		...
		foreach ($rows as $r)
		{
			...
			$c .= utils::composeTextLine(['text' => $r['vyukaNazev'], 'icon' => ($r['vyukaTyp'] === 0) ? 'iconGroupClass' : 'system/iconUser', 'class' => 't1 block']);
			$c .= utils::composeTextLine(['text' => $r['predmetNazev'], 'suffix' => $naplanovano.' min z '.$this->ucebniPlanPredpis[$teachPlanId], 'class' => 'block']);
			$c .= utils::composeTextLine(['text' => $r['ucitelJmeno'], 'class' => 'e10-small block']);
		...
		if ($cnt)
		{
			$this->codeToPlan = utils::composeTextLine(['text' => 'Nezaplánované výuky', 'class' => 'h2 block']);
			$this->codeToPlan .= $c;
		}
	}
