...
class TableMsgsRecipients extends DbTable
{
	public function __construct($dbmodel)
	{
		...
		$this->setName('e10pro.zus.msgsRecipients', 'e10pro_zus_msgsRecipients', 'Příjemci zpráv');
	}
}
...
class ViewMsgsRecipients extends TableView
{
	...
	public function renderRow ($item)
	{
		...
		$listItem ['t2'] = $item['contactName'].': '.$item['contactEmail'];

		$props = [];

		if ($item['sent'])
			$props [] = ['icon' => 'system/iconPaperPlane', 'text' => 'Odesláno '.utils::datef ($item['sentDate'], '%D, %T')];
		else
			$props [] = ['icon' => 'icon-hourglass-half', 'text' => 'Čeká na odeslání'];
    ...
    */
    ...
    array_push ($q, ' contacts.contactEmail AS contactEmail, contacts.contactName');
