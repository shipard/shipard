...
use E10\json;
use \E10\utils, \E10\TableView, \E10\TableViewDetail, \E10\TableForm, \E10\TableViewPanel, \E10\DbTable, \e10\TableFormShow;
 ...
 class TableDatasources extends DbTable
{
	public function __construct($dbmodel)
	{
		...
		$this->setName('e10pro.hosting.server.datasources', 'e10pro_hosting_server_datasources', 'Zdroje dat');
	}
	...
	public function checkBeforeSave(&$recData, $ownerData = NULL)
	{
		...
		{
			// -- ds image
			$image = \E10\Base\getAttachmentDefaultImage($this->app(), 'e10pro.hosting.server.datasources', $recData ['ndx']);
			...
			// -- app DNS record
			$server = $this->db()->query('SELECT * FROM [e10pro_hosting_server_servers] WHERE [ndx] = %i', $recData['server'])->fetch();
			/** @var \e10\DbTable $tableDomainsRecords */
			$tableDomainsRecords = $this->app()->table('e10pro.hosting.server.domainsRecords');

			if ($recData['docState'] === 4000 && $server && isset($recData['dsId1']) && $recData['dsId1'] !== '' && isset($recData['dsId1DomainRec']))
			{
				if (!$recData['dsId1DomainRec'])
				{
					// -- create
					$dnsRec = [
						'domain' => 99, // shipard.app
						'recordType' => 'CNAME',
						'hostName' => $recData['dsId1'],
						'value' => $server['id'] . '.shipard.app',
						'priority' => 0,
						'ttl' => 900,
						'docState' => 4000, 'docStateMain' => 2
					];

					$dnsRecId = $dnsRec['hostName'] . '-' . $dnsRec['value'] . '-' . $dnsRec['priority'] . '-' . $dnsRec['ttl'];
					$dnsRec['versionData'] = sha1($dnsRecId);

					$dnsNdx = $tableDomainsRecords->dbInsertRec($dnsRec);
					$tableDomainsRecords->docsLog($dnsNdx);
					$recData['dsId1DomainRec'] = $dnsNdx;
					$this->db()->query('UPDATE [e10pro_hosting_server_datasources] SET [dsId1DomainRec] = %i', $dnsNdx, ' WHERE [ndx] = %i', $recData['ndx']);
				}
				else
				{
					// -- check
					$dnsRec = $tableDomainsRecords->loadItem($recData['dsId1DomainRec']);
					$newDnsRecValue = $server['id'] . '.shipard.app';
					if ($dnsRec && $dnsRec['value'] !== $newDnsRecValue)
					{
						$update = [
							'value' => $newDnsRecValue,
						];

						$dnsRecId = $dnsRec['hostName'] . '-' . $dnsRec['value'] . '-' . $dnsRec['priority'] . '-' . $dnsRec['ttl'];
						$update['versionData'] = sha1($dnsRecId);
						$this->db()->query('UPDATE [e10pro_hosting_server_domainsRecords] SET ', $update, ' WHERE [ndx] = %i', $recData['dsId1DomainRec']);
						$tableDomainsRecords->docsLog($dnsRec['ndx']);
					}
				}
			}
		}
...
//			"enumValues": {"0": "Zkušební lhůta", "1": "Ostrý provoz", "3": "Expirováno", "5": "Restart"}},
		$labels['condition'][] = [
...
class ViewDatasources extends TableView
{
	...
	public function renderRow ($item)
	{
		...
		if ($item['supportKindName'])
			$props3 [] = ['text' => $item['supportKindName'], 'icon' => 'icon-life-ring', 'class' => 'label label-info'];
		else
			$props3 [] = ['text' => '---', 'icon' => 'icon-life-ring', 'class' => 'label label-danger'];
		...
		if ($item['dsId1'] !== '')
		{
			$dsId = ['text' => $item['dsId1'], 'class' => '', 'icon' => 'icon-at'];

			if (!$item['dsId1DomainRec'])
				$dsId['class'] = 'label label-danger';
	...
	function decorateRow (&$item)
	{
		if (isset($this->dsStats[$item ['pk']]))
		{
			...
			/*
			if ($this->dsStats[$item ['pk']]['cntDocuments12m'] || $this->dsStats[$item ['pk']]['cntDocumentsAll'])
			{
				$info = ['text' => utils::snf($this->dsStats[$item ['pk']]['cntDocuments12m']), 'class' => 'label label-success pull-right', 'icon' => 'icon-file-text-o'];
				if ($this->dsStats[$item ['pk']]['cntDocumentsAll']) // TODO: remove if in next stable version
					$info ['suffix'] = utils::snf($this->dsStats[$item ['pk']]['cntDocumentsAll']);
				$item ['t3'][] = $info;
			}
			*/
			...
			*/
			...
			// -- users
			/*
			if ($this->dsStats[$item ['pk']]['cntUsersAll1m'])
			{
				$info = [
	...
	public function selectRows ()
	{
		...
		array_push ($q, ' sk.name AS supportKindName');
		...
		array_push ($q, ' LEFT JOIN e10pro_hosting_server_servers as servers ON ds.server = servers.ndx');
		array_push ($q, ' LEFT JOIN e10pro_hosting_server_partners as partners ON ds.partner = partners.ndx');
		array_push ($q, ' LEFT JOIN e10pro_hosting_server_supportsKinds AS sk ON ds.supportKind = sk.ndx');
    ...
    // -- aktuální
      ...
      array_push ($q, " AND ds.[docStateMain] = 4");
		...
		if (isset($qv['supportsKinds']))
			array_push ($q, ' AND [supportKind] IN %in', array_keys($qv['supportsKinds']));
	...
	public function createPanelContentQry (TableViewPanel $panel)
	{
		...
		// -- servers
		$servers = $this->db()->query ('SELECT ndx, name FROM e10pro_hosting_server_servers WHERE docStateMain <= 2 AND serverType != 1 ORDER BY name')->fetchPairs ('ndx', 'name');
		$this->qryPanelAddCheckBoxes($panel, $qry, $servers, 'servers', 'Servery');

		// -- supportsKinds
		$sk = $this->db()->query ('SELECT ndx, name FROM e10pro_hosting_server_supportsKinds WHERE docStateMain <= 2 ORDER BY name')->fetchPairs ('ndx', 'name');
		$sk[0] = 'Nenastaveno';
		$this->qryPanelAddCheckBoxes($panel, $qry, $sk, 'supportsKinds', 'Úroveň podpory');
...
class ViewDetailDataSourceUsers extends TableViewDetail
{
	public function createDetailContent ()
	{
		$this->addContentViewer ('e10pro.hosting.server.usersds', 'e10pro.hosting.server.ViewUsersDataSources',
		...
		$this->openForm ();
			...
			$this->openTabs ($tabs, TRUE);
				...
				$this->openTab ();
					...
					$this->addColumnInput ('supportKind');
