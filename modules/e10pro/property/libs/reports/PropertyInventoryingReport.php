...
namespace e10pro\property\libs\reports;
...
/**
 * Class PropertyInventoryingReport
 * @package e10pro\property\libs\reports
 */
class PropertyInventoryingReport extends \e10doc\core\libs\reports\GlobalReport
{
	var $list = [];
	var $listByGroups = [];
	var $tableHeader = [];
	...
	function init()
	{
		...
		$this->addParam ('fiscalPeriod', 'fiscalPeriod', ['flags' => ['enableAll', 'years'], 'title' => 'Pořízení']);
		$this->addMyParams();
	...
	public function createContent()
	{
		...
		$this->createContentGroup('places');
	}

	public function createContentGroup($groupId)
	{
		if (!isset($this->listByGroups[$groupId]))
			return;


		foreach ($this->listByGroups[$groupId] as $group)
		{
			$table = [];
			foreach ($group['items'] as $propertyNdx)
			{
				$table[] = $this->list[$propertyNdx];
			}
			$this->addContent([
				'type' => 'table', 'header' => $this->tableHeader, 'table' => $table,
				'title' => $group['title'],//['text' => $group['title'], 'class' => 'h2tab'],
	...
	protected function loadList()
	{
		...
		$q[] = 'SELECT property.*, types.shortName as typeName, statesPersons.fullName as statePersonName,';
		array_push($q, ' statesPlaces.shortName as statePlaceName, statesPlaces.ndx as statePlaceNdx, statesPlaces.placeParent as statePlaceParentNdx');
		...
		if ($fmNdx != '0')
		{
			e10utils::fiscalPeriodDateQuery($this->app, $q, 'dateStart', $fmNdx);
			$this->setInfo('param', 'Období pořízení', $this->reportParams ['fiscalPeriod']['activeTitle']);
		}
		...
		foreach ($rows as $r)
		{
			...
			$placeNdx = $r['statePlaceNdx'];
			if (!isset($this->listByGroups['places'][$placeNdx]))
			{
				$title = [];
				if ($placeNdx && $r['statePlaceParentNdx'])
				{
					$parentPlaces = [];
					$this->tablePlaces->loadPathUp($r['statePlaceParentNdx'], $parentPlaces);
					$parentPlaces = array_reverse($parentPlaces);

					if (count($parentPlaces))
					{
						$ppTitles = [];
						foreach ($parentPlaces as $pp)
							$ppTitles[] = $pp['fullName'];
					}
					$title[] = ['text' => implode(' → ', $ppTitles), 'class' => 'e10-small block'];
				}
				$title[] = ['text' => ($r['statePlaceName']) ? $r['statePlaceName'] : 'Nepřiřazeno', 'class' => 'h1'];

				$this->listByGroups['places'][$placeNdx] = [
					'title' => $title, 'items' => [],
				];
			}

			$this->listByGroups['places'][$placeNdx]['items'][] = $r['ndx'];
		}
