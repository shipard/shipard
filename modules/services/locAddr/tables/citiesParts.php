...
/**
 * class TableCitiesParts
 */
class TableCitiesParts extends DbTable
{
	public function __construct ($dbmodel)
	{
		parent::__construct ($dbmodel);
		$this->setName ('services.locAddr.citiesParts', 'services_locAddr_citiesParts', 'Části obcí');
	}
	...
	public function createHeader ($recData, $options)
	{
		...
		$idsLabels[] = ['text' => '_'.$recData ['cityPartId'], 'class' => 'label label-primary pull-right'];
...
/**
 * class ViewCitiesParts
 */
class ViewCitiesParts extends TableView
{
	...
	public function selectRows ()
	{
		...
		$q = [];
		array_push ($q, ' SELECT [citiesParts].*,');
    ...
    array_push ($q, ' LEFT JOIN [services_locAddr_laUnits] AS [laUnits0] ON cities.laUnitOwner0 = laUnits0.ndx');
		...
		{
			array_push ($q, ' AND (');
      array_push ($q, '[citiesParts].[fullName] LIKE %s', '%'.$fts.'%');
			array_push ($q, ')');
    }
 ...
 * class FormCityPart
 */
class FormCityPart extends TableForm
{
...
/**
 * Class ViewDetailCityPart
 */
class ViewDetailCityPart extends TableViewDetail
{
