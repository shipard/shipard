...
class SetSizes extends Utility
{
	var $definitions = [];
	...
	{
		$q = 'SELECT * FROM [services_subjects_sizes] WHERE docState != 9800 ORDER BY [fullName], [ndx]';
		...
		foreach ($rows as $r)
		{
			...
			array_push ($qn, ' AND [tableId] = %s', 'services.subjects.sizes');
	...
	public function run()
	{
		...
		foreach ($this->definitions as $def)
		{
			...
			foreach ($subjects as $sub)
			{
				...
				$this->db()->query('UPDATE [services_subjects_subjects] SET [size] = %i', $def['recData']['ndx'], ' WHERE ndx = %i', $subjectNdx);
