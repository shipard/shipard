...
class TablePlaces extends DbTable
{
	public function __construct ($dbmodel)
	{
		...
		$this->setName ('e10.base.places', 'e10_base_places', 'Místa');
	}
	...
	public function loadPathUp ($placeNdx, &$dst)
	{
		$q [] = 'SELECT places.* FROM [e10_base_places] AS places';
		array_push ($q, ' WHERE places.ndx = %i', $placeNdx);
		//array_push ($q,' ORDER BY id, shortName');
		...
		foreach ($rows as $r)
		{
			...
			$this->loadPathUp ($r['placeParent'], $dst);
		}
	}
...
class FormPlace extends TableForm
{
	public function renderForm ()
	{
		...
		$this->openForm ();
			$tabs ['tabs'][] = ['text' => 'Místo', 'icon' => 'x-content'];
			$tabs ['tabs'][] = ['text' => 'Adresa', 'icon' => 'icon-location-arrow'];
	...
	{
		if ($srcTableId === 'e10.base.places' && $srcColumnId === 'placeParent')
		{
			$cp = [
