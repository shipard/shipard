...
class TableDomainsAccounts extends DbTable
{
	public function __construct ($dbmodel)
	{
		...
		$this->setName ('hosting.core.domainsAccounts', 'hosting_core_domainsAccounts', 'Doménové účty');
	}
	...
	public function apiClient ($accountNdx)
	{
		...
		$registrarCfg = $this->app()->cfgItem ('hosting.core.domainsRegistrars.'.$accountRecData['registrar'], NULL);
		if (!$registrarCfg)
		{
			error_log ("registrarCfg not found: `".'hosting.core.domainsRegistrars.'.$accountRecData['registrar']."`");
			return NULL;
		}
...
class ViewDomainsAccounts extends TableView
{
	...
	public function selectRows ()
	{
		...
		$q[] = 'SELECT * FROM [hosting_core_domainsAccounts] ';
...
class FormDomainAccount extends TableForm
{
	...
	{
		$registrarCfg = $this->app()->cfgItem('hosting.core.domainsRegistrars.'.$this->recData['registrar'], NULL);
