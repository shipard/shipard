...
class TableDomains extends DbTable
{
	public function __construct ($dbmodel)
	{
		...
		$this->setName ('hosting.core.domains', 'hosting_core_domains', 'Domény');
	}
...
class ViewDomains extends TableView
{
	...
	public function selectRows ()
	{
		...
		array_push($q, ' FROM [hosting_core_domains] AS [domains]');
		array_push($q, ' LEFT JOIN [e10_persons_persons] AS [owners] ON domains.owner = owners.ndx');
		array_push($q, ' LEFT JOIN [hosting_core_domainsAccounts] AS [accountsReg] ON domains.domainAccount = accountsReg.ndx');
		array_push($q, ' LEFT JOIN [hosting_core_domainsAccounts] AS [accountsDNS] ON domains.domainAccountDNS = accountsDNS.ndx');
...
class ViewDetailDomainRecords extends TableViewDetail
{
	public function createDetailContent ()
	{
		$this->addContent (
			[
				'type' => 'viewer', 'table' => 'hosting.core.domainsRecords', 'viewer' => 'hosting.core.ViewDomainsRecords',
...
class ViewDetailDomainAPI extends TableViewDetail
{
	public function createDetailContent ()
	{
		$this->addDocumentCard('hosting.core.dc.DomainApi');
	}
}
