...
| 06b | (viz Fáze 10) | ✅ Hotovo | Cílený re-import — dodáno jako CLI `forget --entity` (smaže mapping jen pro jednu entitu, vynutí re-import bez resetu celé mapy). Implementováno v rámci Fáze 10 (commit `5133678b`). |
| 07a | [07a-attachments-client.md](07a-attachments-client.md) | ✅ Hotovo | Obecný entity-agnostický klient pro import příloh (`e10_attachments_files`) přes REST upload — `(oldTableId, oldRecId)` → `(newTableIdNumeric, newRecordId)`. Prvním konzumentem je MailRunner (07b). |
| 07b | [07b-mail.md](07b-mail.md) | ✅ Hotovo | Import došlé pošty (`wkf_core_issues`, issueType=1) → `core_mail_incoming_messages`: mapování sekcí na schránky, PDF přílohy (07a), vazba na doklady (`e10docs-inbox`). |
| 07c | [07c-mail-fixes.md](07c-mail-fixes.md) | ✅ Hotovo | Opravy importu pošty po reálném běhu: `docState` z původního `issues.docState` (nezávisle na vazbě), HTML tělo do `body_html` (heuristika), vazba na doklad i z `tableNdx`/`recNdx`. |
| 08 | [08-accounts.md](08-accounts.md) | ✅ Hotovo | Import účtového rozvrhu (`e10doc_debs_accounts`) → `economy_accounting_accounts` přes generický CRUD (subclass `BaseCodebookRunner`, idempotence přes LocalIdMap). |
