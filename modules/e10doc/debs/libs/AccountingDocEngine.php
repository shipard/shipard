...
/**
 * class AccountingDocEngine
 */
class AccountingDocEngine extends Utility
{
  var $enabledAccRings = NULL;
  var $docHead;
  var $docFiscalMonth;
  var $docTaxRegCfg;
  ...
  public function incRow ($row, $enableZeroMoney = 0)
  {
  ...
  protected function createJournalRowsExts ()
  {
    $accExts = $this->app()->cfgItem('e10doc.debs.accExts', NULL);
    if (!$accExts)
      return;

    foreach ($accExts as $ae)
    {
      if (!isset($ae['docTypes']) || !in_array($this->docHead['docType'], $ae['docTypes']))
        continue;
      $doEngine = $this->app()->createObject($ae['engine']);

      if ($doEngine)
      {
        $doEngine->doDocument ($this);
      }
    }
  }
  ...
  public function save ()
  {
    ...
    forEach ($this->docJournal as $r)
    {
      ...
      if ($this->enabledAccRings && !in_array($r['accRing'], $this->enabledAccRings))
        continue;
