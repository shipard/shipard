...
i=0;i<input.files.length;i++){var
file=input.files[i];var
url=e10.server.httpServerRoot+"/upload/e10.base.attachments/"+table+'/'+pk+'/'+file.name;e10AttWidgetUploadOneFile(url,file,infoPanel,i);}}function
...
c="<div class='e10-form-get-number' id='"+e10.form.gnId+"'>";c+="<table class='e10-get-number-keyboard'>";c+="<tr>";c+="<td class='c e10-trigger-gn'><i class='fa fa-times'></i></td><td class='m' colspan='3'>";if(e10.form.gnOptions.title)c+="<div class='title'>"+e10.escapeHtml(e10.form.gnOptions.title)+"</div>";if(e10.form.gnOptions.subtitle)c+="<div class='e10-small'>"+e10.escapeHtml(e10.form.gnOptions.subtitle)+"</div>";c+="</td>";c+="</tr>";c+="<tr>";c+="<td class='d e10-trigger-gn' colspan='3'></td><td class='b e10-trigger-gn'><i class='fa fa-arrow-circle-left'></i></td>";c+="</tr>";c+="<tr>";c+="<td class='n e10-trigger-gn'>7</td><td class='n e10-trigger-gn'>8</td><td class='n e10-trigger-gn'>9</td><td class='ok e10-trigger-gn' rowspan='4'><i class='fa fa-check'></i></td>";c+="</tr>";c+="<tr>";c+="<td class='n e10-trigger-gn'>4</td><td class='n e10-trigger-gn'>5</td><td class='n e10-trigger-gn'>6</td>";c+="</tr>";c+="<tr>";c+="<td class='n e10-trigger-gn'>1</td><td class='n e10-trigger-gn'>2</td><td class='n e10-trigger-gn'>3</td>";c+="</tr>";c+="<tr>";c+="<td class='n e10-trigger-gn' colspan='2'>0</td><td class='n e10-trigger-gn'>,</td>";c+="</tr>";c+="</table>";c+="</div>";$('body').append(c);var
...
row='<tr'+' data-pk="'+item.pk+'"'+' data-quantity="'+quantity+'"'+' data-price="'+item.price+'"'+' data-totalprice="'+totalPrice+'"'+'>';row+='<td class="e10-terminal-action" data-action="remove-row"><i class="fa fa-times"></i></td>';row+='<td class="item">'+'<span class="t">'+e10.escapeHtml(item.name)+'</span>'+'<br>'+'<span class="e10-small i e10-terminal-action" data-action="row-price-item-change">'+quantity+' '+item.unitName+' á '+priceStr+' = <b>'+totalPriceStr+'</b>'+'</span>'+'</td>';row+='<td class="q number">'+quantity+'</td>';row+='<td class="e10-terminal-action" data-action="quantity-plus"><i class="fa fa-plus"></i></td>';row+='<td class="e10-terminal-action" data-action="quantity-minus"><i class="fa fa-minus"></i></td>';row+='</tr>';var
...
idd=new
Date().valueOf();var
url=$(this).attr('data-src')+'?xyz='+idd;$(this).attr('src',url);});e10.widgets.macLan.badgesTimer=setTimeout(e10.widgets.macLan.reloadBadges,20000);};e10client.prototype.widgets.macLan.reloadAlerts=function(){if(e10.widgets.macLan.alertsTimer){clearTimeout(e10.widgets.macLan.alertsTimer);}var
...
cameraId=imgElement.attr('data-camera');if(imgElement.hasClass('zoomed'))imgElement.attr("src",origPicFileName).parent().attr("data-pict",origPicFileName);else
imgElement.attr("src",picFileName).parent().attr("data-pict",origPicFileName);if(e10.widgets.macVs.smartMainElement!==null&&cameraId===e10.widgets.macVs.smartActiveCamera){$('#e10-vs-smart-main-img').attr('src',origPicFileName);}if(data[ii].error)imgElement.addClass('e10-error');else
...
badges=b64DecodeUnicode(e.attr('data-badges-code'));$('#e10-vs-smart-main-img').parent().append(badges);}};e10client.prototype.widgets.macVs.zoomPicture=function(e){if(e.hasClass('zoomed')){e.removeClass('zoomed');e.parent().parent().removeClass('zoomed');}else{e.addClass('zoomed');e.parent().parent().addClass('zoomed');}};e10client.prototype.widgets.macVs.zoomMainPicture=function(e){var
primaryBox=e.parent().find('div.e10-wvs-smart-primary-box');if(e.hasClass('zoomed')){e.removeClass('zoomed');primaryBox.removeClass('zoomed');}else{e.addClass('zoomed');primaryBox.addClass('zoomed');}};e10client.prototype.widgets.macVs.setVideo=function(e){var
camUrl=e.attr('data-cam-url');var
cameraId=e.attr('data-camera');var
bfn=e.attr('data-bfn');var
...
c='';c.length<32;)c+=Math.random().toString(36).substr(2,1);e10.deviceId=c;e10.options.set('deviceId',e10.deviceId);}e10.options.loadAppSettings();e10.server.setHttpServerRoot(httpApiRootPath);e10.wss.init();if('serviceWorker'in
