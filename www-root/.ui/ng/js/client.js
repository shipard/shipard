...
ShipardTableViewer
extends
ShipardWidget{elmViewerLines=null;elmViewerRows=null;elmViewerDetail=null;elmViewerDetailContent=null;elmViewerDetailHeader=null;elmViewerDetailTabs=null;doWidgetResponse(data){console.log("ShipardTableViewer::doWidgetResponse");super.doWidgetResponse(data);}init(e){console.log("ShipardTableViewer::init");super.init(e);this.rootElm.style.display='grid';const
id=e.getAttribute('id');this.elmViewerLines=document.getElementById(id+'Items');this.elmViewerRows=this.elmViewerLines.parentElement;this.elmViewerRows.addEventListener('scroll',(event)=>{this.doScroll(event)});this.elmViewerDetail=this.rootElm.querySelector('div.detail');this.elmViewerDetailContent=this.elmViewerDetail.querySelector('div.content');this.elmViewerDetailHeader=this.elmViewerDetail.querySelector('div.header');this.elmViewerDetailTabs=this.elmViewerDetail.querySelector('div.tabs');this.on(this,'click','div.rows-list.mainViewer>div.r',function(e,ownerWidget,event){this.rowClick(e,event)}.bind(this));}doAction(actionId,e){console.log("viewerAction",actionId);switch(actionId){case'newform':return this.actionNewForm(e);case'viewerTabsReload':this.viewerTabsReload(e);case'detailSelect':this.detailSelect(e);}return super.doAction(actionId,e);}doScroll(event){const
...
activeRowElement=this.elmViewerLines.querySelector('.active');this.detailOpen(activeRowElement);}}return 0;}detailOpen(activeRowElement){const
...
apiParams={};apiParams['requestType']='dataViewerDetail';apiParams['actionId']='loadDetail';apiParams['table']=tableId;apiParams['viewId']=viewId;apiParams['detailId']=detailId;apiParams['pk']=rowNdx;this.detectValues(apiParams);console.log("API-CALL-DETAIL-OPEN",apiParams);var
url='api/v2';shc.server.post(url,apiParams,function(data){console.log("--api-call-detail-success--",data);this.doDetailOpenResponse(data);}.bind(this),function(data){console.log("--api-call-error--");}.bind(this));}doDetailOpenResponse(data){this.setInnerHTML(this.elmViewerDetailHeader,data.response.hcHeader);this.setInnerHTML(this.elmViewerDetailContent,data.response.hcContent);}df2FillViewerLines(){var
