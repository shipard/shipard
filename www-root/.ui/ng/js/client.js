...
ShipardWidget{rootElm=null;rootId='';numPad=null;init(rootElm){this.rootElm=rootElm;this.rootId=this.rootElm.getAttribute('id');this.on(this,'click','.shp-widget-action',function(e,ownerWidget,event){ownerWidget.widgetAction(e,event)});this.on(this,'click','.shp-widget-action>i',function(e,ownerWidget,event){ownerWidget.widgetAction(e.parentElement,event)});}widgetAction(e,event){let
actionId=e.getAttribute('data-action');this.doAction(actionId,e);event.stopPropagation();}doAction(actionId,e){switch(actionId){case'inline-action':return this.inlineAction(e);case'select-main-tab':return this.selectMainTab(e);case'select-simple-tab':return this.selectSimpleTab(e);case'open-popup':return this.openPopup(e);case'open-modal':return this.openModal(e);case'closeModal':return this.closeModal(e);case'treeListGroupOC':return this.treeListGroupOC(e);}return 0;}inlineAction(e){if(e.getAttribute('data-object-class-id')===null)return;var
...
newEnvelope=document.createElement('data-modal-env');newEnvelope.setAttribute('data-request-type','dataModal');for(const
oneParamId
in
modalParams)newEnvelope.setAttribute('data-action-param-'+oneParamId,modalParams[oneParamId]);newEnvelope.id='shc_meid_'+shc.counter++;newEnvelope.innerHTML="čekejte, prosím, data se načítají...";document.body.appendChild(newEnvelope);switch(modalType){case'viewer':console.log('Viewer!');break;}let
...
url='api/v2';shc.server.post(url,apiParams,function(data){console.log("--api-call-MODAL-success--",data);this.doWidgetModalResponse(data,newEnvelope.id);}.bind(this),function(data){console.log("--api-call-MODAL-error--");}.bind(this));return 0;}closeModal(e){this.rootElm.parentElement.remove();return 0;}openPopup(e){const
...
valueKey=inputs[i].getAttribute('name');data[valueKey]=inputs[i].value;}}doWidgetResponse(data){if(data['response']!==undefined&&data['response']['uiData']!==undefined)shc.applyUIData(data['response']['uiData']);console.log(data);}doWidgetModalResponse(data,targetElementId){if(data['response']!==undefined&&data['response']['uiData']!==undefined)shc.applyUIData(data['response']['uiData']);var
targetElement=document.getElementById(targetElementId);this.setInnerHTML(targetElement,data.response.hcFull);if(data.response.objectType==='dataView')initWidgetTableViewer(data.response.objectId);else{console.log("init-other-widget");let
e=document.getElementById(data.response.objectId);e.shpWidget=new
ShipardWidget();e.shpWidget.init(e);}}on(ownerWidget,eventType,selector,callback){this.rootElm.addEventListener(eventType,function(event){if(event.target.matches(selector)){callback.call(event.target,event.target,ownerWidget,event);}});}onClick(ownerWidget,selector,callback){this.on(ownerWidget,'click',selector,callback)};nf(n,c){var
...
id=e.getAttribute('id');this.elmViewerLines=document.getElementById(id+'Items');this.elmViewerRows=this.elmViewerLines.parentElement;this.elmViewerRows.addEventListener('scroll',(event)=>{this.doScroll(event)});}doAction(actionId,e){switch(actionId){case'newform':return this.actionNewForm(e);}return super.doAction(actionId,e);}doScroll(event){const
...
if(data['response']['type']==='refreshData'){this.appendNextData(data,1);return;}}appendNextData(data,clear){if(clear!==undefined){this.elmViewerLines.innerHTML=data['response']['hcRows'];}else{this.elmViewerLines.removeChild(this.elmViewerLines.lastElementChild);this.elmViewerLines.innerHTML+=data['response']['hcRows'];}this.elmViewerLines.setAttribute('data-rowspagenumber',data['response']['rowsPageNumber']);this.elmViewerRows.setAttribute('data-loadonprogress',0);}}function
...
extends
ShipardWidget{mainAppContent=null;elmAppTitle=null;init(e){this.mainAppContent=document.getElementById('shp-main-app-content');this.elmAppTitle=document.getElementById('shp-app-hdr-title');console.log("ShipardWidgetApplication::init");super.init(e);this.initContent();}initContent(){}doAction(actionId,e){switch(actionId){case'loadAppMenuItem':return this.loadAppMenuItem(e);case'toggleContent':return this.fullScreenContentToggle(e);}return super.doAction(actionId,e);}fullScreenContentToggle(e){if(this.rootElm.classList.contains('full-screen-content-on'))this.fullScreenContentOff();else
this.fullScreenContentOn();return 0;}fullScreenContentOn(){this.rootElm.classList.remove('full-screen-content-off');this.rootElm.classList.add('full-screen-content-on');}fullScreenContentOff(){this.rootElm.classList.remove('full-screen-content-on');this.rootElm.classList.add('full-screen-content-off');}doWidgetResponse(data){super.doWidgetResponse(data);}loadAppMenuItem(e){console.log('loadAppMenuItem222',e);const
