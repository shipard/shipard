...
WidgetVendM
extends
ShipardWidgetBoard{elmTableBoxes=null;elmSBDisplayItemName=null;elmSBDisplayItemPrice=null;elmSBDisplayItemBtns=null;elmCardCodeInput=null;itemName='';itemNdx=0;personNdx=0;itemPrice=0;itemBoxId='';init(rootElm){super.init(rootElm);this.elmTableBoxes=this.rootElm.querySelector('table.vmSelectBox');this.elmSBDisplayItemName=document.getElementById('vm-select-box-display-item-name');this.elmSBDisplayItemPrice=document.getElementById('vm-select-box-display-item-price');this.elmSBDisplayItemBtns=document.getElementById('vm-select-box-display-item-btns');this.elmCardCodeInput=document.getElementById('vm-card-code-input');this.elmCardCodeInput.addEventListener("keypress",function(event){if(event.key==="Enter"){event.preventDefault();this.validateCardCode();}}.bind(this));this.selectBoxDisplay();}doAction(actionId,e){console.log("VM-ACTION: ",actionId);switch(actionId){case'vmSelectBox':return this.selectBox(e);case'vmBuyGetCard':return this.buyGetCard(e);case'vmBuyCancel':return this.buyCancel(e);}return super.doAction(actionId,e);}doApiObjectResponse(data){if(data.response.classId==='vendms-validate-code'){this.elmHide(this.rootElm.querySelector('div.statusInvalidCode'));this.elmHide(this.rootElm.querySelector('div.statusInvalidCredit'));this.elmHide(this.rootElm.querySelector('div.statusCardVerify'));if(data.response.validPerson!==1){this.elmShow(this.rootElm.querySelector('div.statusInvalidCode'));return;}this.personNdx=data.response.personNdx;this.setVMMode('do-buy');this.doBuyCreateInvoice();return;}if(data.response.classId==='vendms-create-invoice'){if(data.response.success!==1){return;}this.doBuyEjectItem();}console.log("VM_doApiObjectResponse",data);}selectBox(e){let
oldBoxId='';let
...
newBoxId=e.getAttribute('data-box-id');if(newBoxId===oldBoxId){this.itemName='';this.itemPrice=0;this.itemNdx=0;this.itemBoxId='';this.selectBoxDisplay();return 1;}e.classList.add('active');this.itemName=e.getAttribute('data-item-name');this.itemPrice=parseFloat(e.getAttribute('data-item-price'));this.itemNdx=parseInt(e.getAttribute('data-item-ndx'));this.itemBoxId=newBoxId;this.selectBoxDisplay();return 1;}selectBoxDisplay(){if(this.itemNdx===0){this.elmSBDisplayItemName.innerText='Vyberte, co si chcete koupit....';this.elmSBDisplayItemPrice.innerText='';this.elmHide(this.elmSBDisplayItemBtns);return;}this.elmSBDisplayItemName.innerText=this.itemName;this.elmShow(this.elmSBDisplayItemBtns);}buyGetCard(e){console.log('buyGetCard');this.setVMMode('get-card');return 1;}buyCancel(e){console.log('buyCancel');this.setVMMode('select');return 1;}validateCardCode(){let
...
if(mode==='do-buy'){document.getElementById('vm-mode-buy-in-progress').classList.add('active');}}doBuyCreateInvoice(){console.log('create_invoice');this.apiCallObject('vendms-create-invoice',{'itemNdx':this.itemNdx,'personNdx':this.personNdx});}doBuyEjectItem(){location.reload();}}function
