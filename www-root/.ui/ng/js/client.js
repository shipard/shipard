...
if(mode==='info-card'){this.elmHide(this.rootElm.querySelector('div.statusInvalidCode'));this.elmHide(this.rootElm.querySelector('div.statusInvalidCredit'));this.elmHide(this.rootElm.querySelector('div.statusCardVerify'));document.getElementById('vm-mode-info-card').classList.add('active');}}doBuyCreateInvoice(){if(this.buyTimeout!=0)clearTimeout(this.buyTimeout);this.buyTimeout=0;shc.mqtt.publish(1,this.topicMotorsMatrix,this.motorsMatrixValue);this.apiCallObject('vendms-create-invoice',{'itemNdx':this.itemNdx,'boxNdx':this.itemBoxNdx,'personNdx':this.personNdx});}doRfidReader(cardId){console.log("RFID1: ",cardId);if(this.vmMode==='select'){if(this.setupModeCards!==null&&this.setupModeCards.indexOf(cardId)>=0){document.location.href=this.setupUrl;return;}this.setVMMode('info-card');this.validateCardCode(cardId);return;}if(this.vmMode==='get-card'){this.validateCardCode(cardId);return;}}onMqttMessage(serverIndex,topic,payload){console.log("vendms - onMqttMessage: ",topic,payload);if(topic===this.rfidReaderTopic){this.doRfidReader(payload['value']);}if(topic==this.tempBottomSensorTopic){document.getElementById('vm-temp-bottom').innerText=parseFloat(payload['value'])+' °C';}console.log("topicBusy: ",this.topicMotorsBusy);if(topic==this.topicMotorsBusy){console.log("YES, busy...");if(parseInt(payload['value'])==0){console.log("!!!RELOAD");location.reload();}}}}function
