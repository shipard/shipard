...
i=0;i<inputs.length;++i){console.log("INPUT: ",inputs[i]);const
...
id=e.getAttribute('id');this.elmViewerLines=document.getElementById(id+'Items');this.elmViewerRows=this.elmViewerLines.parentElement;this.elmViewerRows.addEventListener('scroll',(event)=>{this.doScroll(event)});this.on(this,'click','div.rows-list.mainViewer>div.r',function(e,ownerWidget,event){this.rowClick(e,event)}.bind(this));}doAction(actionId,e){console.log("viewerAction",actionId);switch(actionId){case'newform':return this.actionNewForm(e);case'viewerTabsReload':this.viewerTabsReload(e);}return super.doAction(actionId,e);}doScroll(event){const
...
oldActiveTabElement=itemElement.querySelector('.active');oldActiveTabElement.classList.remove('active');e.classList.add('active');this.refreshData(e);}viewerRefreshLoadNext(){const
tableName=this.rootElm.getAttribute("data-table");if(!tableName)return;this.df2FillViewerLines();}rowClick(e,event){let
oldActiveRowElement=this.elmViewerLines.querySelector('.active');if(oldActiveRowElement)oldActiveRowElement.classList.remove('active');e.classList.add('active');console.log("___ROW_CLICK2___",e);}df2FillViewerLines(){var
