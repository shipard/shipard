...
class DataPackageInstaller extends \E10\Utility
{
	...
	public function run ()
	{
		$this->installPackage($this->pkgFileName);
	...
	public function installPackage ($pkgFileName)
	{
		if (is_array($pkgFileName))
			$pkg = $pkgFileName;
		else
		{
	...
	public function addAttachments ($ownerRecData, $ownerTable, $attachments)
	{
		foreach ($attachments as $att)
		{
			...
			{
				\E10\Base\addAttachments($this->app, $ownerTable->tableId(), $ownerRecData['ndx'], __SHPD_MODULES_DIR__ . $att['fileName'], '');
				$this->cntAttachments++;
			}
		}
	}
	...
	public function installPackage_Includes ($pkg)
	{
		...
		foreach ($pkg['includes'] as $packageId)
		{
			...
			$this->installPackage($fullFileName);
		}
	}
	...
	function checkColumnValues($table, &$recData)
	{
		...
		foreach ($recData as $colId => $colValue)
		{
			...
			if (!$colDef)
			{
				error_log ("ERROR: DataPackageInstaller::checkColumnValues > column `$colId` not exist ".json_encode($recData));
				continue;
			}
