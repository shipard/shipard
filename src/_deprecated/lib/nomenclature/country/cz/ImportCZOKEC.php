 ...
 * @package lib\nomenclature\country\cz
 *
 * http://apl.czso.cz/iSMS/klasdata.jsp?kodcis=80022
 *
 */
class ImportCZOKEC extends ImportNomenclature
{
	...
	public function run()
	{
		if (!$this->downloadFile($this->srcUrl, 'nomenc-cz-okec.xml'))
			return;

		$nomencType = $this->db()->query('SELECT * FROM [e10_base_nomencTypes] WHERE [id] = %s', 'cz-okec')->fetch();
		if ($nomencType)
	...
	protected function importItem ($item, $ownerItem, $level, $mainItem)
	{
		...
		$order = sprintf('%010d', $this->cnt);
		$this->cnt++;
		$newItem = [
				'nomencType' => $this->nomecTypeNdx,
				'id' => 'cz-okec-'.$item['CHODNOTA'],
		...
		$exist = $this->db()->query('SELECT * FROM [e10_base_nomencItems] WHERE [nomencType] = %i', $newItem['nomencType'],
				'AND [id] = %s', $newItem['id'], ' AND [level] = %i', $level)->fetch();
		if ($exist)
		{
