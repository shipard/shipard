...
| **Saldokonto** | Kdo komu kolik dluží, po partnerech. Každý řádek je jeden případ — faktura a její úhrady se stejným variabilním symbolem v jednom účetním období — se zůstatkem a splatností; dluhy po splatnosti jsou zvýrazněné. Seznam se otevře s filtrem **Období** nastaveným na aktuální rok; volba **— vše —** ukáže všechna období. Filtr **Typ** odděluje dluhy, přeplatky a platby bez faktury, **Včetně uzavřených** ukáže i vyrovnané případy. Tlačítko **Pohyby případu** otevře jednotlivé předpisy a úhrady případu v jeho období | — |
| **Saldo pohyby** | Jednotlivé předpisy a úhrady saldokonta, po partnerech, se zůstatkem případu, do kterého patří. Stejný filtr **Období** s výchozím aktuálním rokem | — |
